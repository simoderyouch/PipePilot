...
stage_pull() {
    ...
    if [[ -d "$PROJECT_PATH/.git" ]]; then
        ...
        current_branch="$(capture_in_project git branch --show-current || true)"
        commit_hash="$(capture_in_project git rev-parse --short HEAD || true)"
        author="$(capture_in_project git log -1 --pretty=format:'%an' || true)"
        log_info "[GIT] Pull OK -- branch ${current_branch:-detached} -- commit ${commit_hash:-unknown} -- author ${author:-unknown}"
        status_line "[GIT] branch=${current_branch:-detached} commit=${commit_hash:-unknown} author=${author:-unknown}"
    else
    ...
    detect_project_type
    status_line "[DETECT] project_type=$PROJECT_TYPE"
    return "$OK"
}
